	...
	<body>
		...
		document.getElementById("form-addUser").addEventListener("submit", function(event) {
			...
			if(fullname.trim()===""){
		 	    ...
		 	    document.getElementById("erro-fullname").innerHTML ="Hãy điền họ và tên.";
		 	    return;
			}
			if(username.trim()===""){
				 event.preventDefault(); 
			    document.getElementById("erro-username").innerHTML ="Hãy điền tên tài khoản.";
		 	    return;

				}
			if(!validateEmail(email)){
		  		 event.preventDefault(); 
		   	    document.getElementById("erro-email").innerHTML ="Email không hợp lệ";
		 	    return;

		  		}
			if(!validatePassword(password)){
		 		 event.preventDefault(); 
		  	    document.getElementById("erro-password").innerHTML ="Mật khẩu không hợp lệ";
		 	    return;

		 		}
			if(address.trim()===""){
				 event.preventDefault(); 
		 	    document.getElementById("erro-address").innerHTML ="Hãy thêm địa chỉ";
		 	    return;

				}
			if(numberphone.trim()===""){
				 event.preventDefault(); 
		 	    document.getElementById("erro-numberphone").innerHTML ="Hãy thêm số điện thoại";
		 	    return;
